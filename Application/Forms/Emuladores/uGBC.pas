...
procedure TformGBC.CarregaRoms;
var
  Diretorio: String;
...
begin
  Diretorio := DiretorioPadrao + 'GBC';

  if not TDirectory.Exists(Diretorio) then
  begin
    sbPrincipal.Visible := False;
    pnlPrincipal.Caption := 'Nenhuma Rom de GBC';
    Exit;
  end;

  Arquivos := TDirectory.GetFiles(Diretorio, '*.gbc', TSearchOption.soTopDirectoryOnly);
...
procedure TformGBC.BotaoClick(Sender: TObject);
var
  Botao: TMyCustomPanel;
  Comando: string;
begin
  ...
  '--core mgba_libretro ' +
  '--system 6 ' +
  '--game "' + DiretorioPadrao + '\GBC\' + Botao.Caption + '.gbc"';
  // Executa o comando no CMD
  ShellExecute(0, 'open', 'cmd.exe', PChar('/C ' + Comando), nil, SW_SHOWNORMAL);
end;

procedure TformGBC.BotaoEnter(Sender: TObject);
var
  Botao: TMyCustomPanel;
begin
  Botao := Sender as TMyCustomPanel;
...
procedure TformGBC.BotaoLeave(Sender: TObject);
var
  Botao: TMyCustomPanel;
begin
  Botao := Sender as TMyCustomPanel;
...
procedure TformGBC.FormCreate(Sender: TObject);
begin
  DiretorioPadrao := PegaDiretorio;
  CarregaRoms;
end;

procedure TformGBC.FormResize(Sender: TObject);
var
  I: Integer;
begin
...
procedure TformGBC.FormShow(Sender: TObject);
begin
  //Trata Painel
  pnlPrincipal.Color := RGB(40, 40, 40);
end;

procedure TformGBC.btnVoltarClick(Sender: TObject);
begin
  TformPrincipal(Owner).TrocaForm('Nintendo');
end;

initialization
  RegisterClass(TformGBC);  // Registra a classe para uso dinâmico
