...
procedure TformNES.CarregaRoms;
var
  Diretorio: String;
...
begin
  Diretorio := DiretorioPadrao + 'NES';

  if not TDirectory.Exists(Diretorio) then
  begin
    sbPrincipal.Visible := False;
    pnlPrincipal.Caption := 'Nenhuma Rom de NES';
    Exit;
  end;

  Arquivos := TDirectory.GetFiles(Diretorio, '*.nes', TSearchOption.soTopDirectoryOnly);
...
procedure TformNES.BotaoClick(Sender: TObject);
var
  Botao: TMyCustomPanel;
  Comando: string;
begin
  ...
  '--core mesen_libretro ' +
  '--system 7 ' +
  '--game "' + DiretorioPadrao + '\NES\' + Botao.Caption + '.nes"';
  // Executa o comando no CMD
  ShellExecute(0, 'open', 'cmd.exe', PChar('/C ' + Comando), nil, SW_SHOWNORMAL);
end;

procedure TformNES.BotaoEnter(Sender: TObject);
var
  Botao: TMyCustomPanel;
begin
  Botao := Sender as TMyCustomPanel;
...
procedure TformNES.BotaoLeave(Sender: TObject);
var
  Botao: TMyCustomPanel;
begin
  Botao := Sender as TMyCustomPanel;
...
procedure TformNES.FormCreate(Sender: TObject);
begin
  DiretorioPadrao := PegaDiretorio;
  CarregaRoms;
end;

procedure TformNES.FormResize(Sender: TObject);
var
  I: Integer;
begin
...
procedure TformNES.FormShow(Sender: TObject);
begin
  //Trata Painel
  pnlPrincipal.Color := RGB(40, 40, 40);
end;

procedure TformNES.btnVoltarClick(Sender: TObject);
begin
  TformPrincipal(Owner).TrocaForm('Nintendo');
end;

initialization
  RegisterClass(TformNES);  // Registra a classe para uso dinâmico
